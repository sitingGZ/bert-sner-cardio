...
RESULT_PATH_cardio = BASE_PATH.joinpath("cardiode/trained_muchmore").resolve()
...
HOMEPATH = {'span': 'Overview', 'href':'/overview'}
...
def return_json_cardio_dict(tsv_id,result_path, prediction_file_paths, classification_file_paths):
    ...
    return prediction_dict, classification_dict, None
...
def get_sentence_scores_by_type_cardio(scores_dict, tsv_sentences, sec_tab, sem_types):
    """
...
def make_types_checkbox(group_colors, multi_layer_suggested_terms):
    ...
    for group, types in multi_layer_suggested_terms.items():
        semantic_types_checkbox_groups.append(dbc.Label(group, id='-'.join(group.lower().split()), style={'font-size':'2rem','color': group_colors.get(group).text.value}))
        current_checkbox_group = dbc.Checklist(
            ...
            id="{}-group".format('-'.join(group.lower().split())),
            switch=True)
        semantic_types_checkbox_groups.append(current_checkbox_group)
    
    return dmc.Stack(children=semantic_types_checkbox_groups)
...
def make_select_types_to_explain(sec_scores_dict):
    ...
    return dcc.Dropdown(id='types-options', options = [{'label':ty, 'value':ty} for ty in categories_flat], multi=True)
...
select_domain = html.Div([
    html.H4('Domain'), 
    dcc.Dropdown([d for d in CLINIC_DOMAIN.values()], id='select-domain', multi=False)
    ], style= {'textAlign': 'left'})

#select_document = html.Div([html.H5('Document ID'),dcc.Dropdown([], id="select-document-id", multi=False)],style= {'textAlign': 'left'})    
select_document = html.Div(children = [html.H4('Document ID and Section'), html.Div(id="select-document-id", children=[dcc.Dropdown(options=[], id='selected-id')])], style={'textAlign': 'left'})

# Row 2
section_checklist =  dbc.Checklist(id='cardio-section-checklist', options = [], value = [])  
   
# Row 3
html_div_children = []
html_div_dropdown_ids = {}
left_container_children = [select_domain, html.Br(), select_document,html.Br(), 
                                section_checklist,
                                 html.Hr()]
...
container_vis = dmc.Card(id = "container-vis", children= 
                         [ dmc.CardSection(dmc.Tabs(id='container-vis-tabs', 
                                                    orientation="horizontal", variant='pills',
                                                    persistence_type='local'),inheritPadding=True, py='sx')], 
                         style={"height": "auto"},withBorder=True,shadow="sm", radius="md", )

# Explanation container
container_exp = html.Div(id = "container-exp",  style = {'borderWidth': '1px','borderStyle': 'dotted',
            'borderRadius': '2px',"padding": "1rem 1rem", "background-color": "#ffffff","height": "auto", "overflow": "scroll" } )

        
selected_types = html.Div(id='select-types-explain', children =  dcc.Dropdown(id='types-options'))
#dcc.Dropdown(id='select-types-explain', options = [{'label':label, 'value':label} for label in SUGGESTED_TYPES_ENT + SUGGESTED_TYPES_ATT], value = SUGGESTED_TYPES_ENT[0], multi=False)
container_exp_header = dmc.Text("The difference in model prediction confidence when changing the semantic type preceding the sentence", 
                                                  id='prediction-interpret', 
                                                  size='xl', 
                                                  weight=600, 
                                                  variant="gradient",
                                                gradient={"from": "red", "to": "yellow", "deg": 145}) 
                                #dbc.Col([selected_types])])

container_exp_card = dmc.Card(children=[
                        dmc.CardSection(container_exp_header, 
            inheritPadding=True,py="xs",), 
                        dmc.CardSection(selected_types,  inheritPadding=True, py='xs'),
...
#            id = "label-badge-group-ent", align='flex-start', justify = 'flex-start', spacing ='xs')
...
default_types_risk = dbc.Row([dbc.Col(make_switch(label, colors_suggested_risk[t],t, value=False)) for t, label in RISK_FACTORS.items()],
            id = "label-badge-group-risk", )

tooltips = {}
     
third_container_children_old = [default_types_ent,  
                            html.Hr(), 
                            html.Br(), 
                            dbc.Label('Factuality', id='fact', style={'font-size':'2rem'}, color='primary'), 
                            default_types_fact,  
                            html.Hr(), 
                            html.Br(), 
                            dbc.Label('Temporal (Event)', id='temporal', style={'font-size':'2rem'}, color='primary'), 
                            default_types_temporal,  
                            html.Hr(), 
                            html.Br(), 
                            dbc.Label('Risk Factor', id='risk', style={'font-size':'2rem'}, color='primary'), 
                            default_types_risk]
...
store_tsv_sentences = dcc.Store(id='tsv-or-line-sentences-memory', storage_type='session')
store_section_tab = dcc.Store(id='section-tab-memory', storage_type = 'local')
store_section_sentence = dcc.Store(id='section-sentence-memory', storage_type='session')

store_prediction_files = dcc.Store(id="prediction-files-memory", storage_type="session")
store_classification_files = dcc.Store(id="classification-files-memory", storage_type = "session")
store_prediction_dict = dcc.Store(id='prediction-memory', storage_type='local')
store_classification_dict = dcc.Store(id='classification-memory', storage_type='local')
store_vis_selected_types = dcc.Store(id='vis-selected-types-memory', storage_type='local')
store_spans_all_sentences = dcc.Store(id ='spans-all-sentences-memory', storage_type='local')

title = html.Div(id='interface-title')
...
@callback(
    Output('interface-title', 'children'),
    Input('select-domain', 'value')
)
def make_interface_title(domain):
    html_title = Purify("<h4>Klinischer Entitäten ({}) Erkennung mit Einheitlichen Medizinischen Semantischen Typen </h4>".format(domain))
    #print(type(html_title))
    return html_title

#print(text_ids)

@callback(Output('vis-selected-types-memory', 'data'), [Input("{}-group".format('-'.join(group.lower().split())), 'value') for group, types in multi_layer_suggested_terms.items()])
def return_selected_types(group1, group2, group3, group4, group5, group6, group7):
    return group1+group2+group3+group4+group5+group6+group7

@callback([Output('prediction-files-memory', 'data'), 
           Output('classification-files-memory', 'data'),
           Output('select-document-id', 'children')], 
          Input('select-domain', 'value'), prevent_initial_call = True)
def return_saved_results(domain):
    ...
    if domain:
        if domain == 'Kardiologie':
            ...
            dropdown_list.append(dcc.Dropdown(options=[{'label':d, 'value':d} for d in text_ids], id='selected-id'))
        
        else:
            prediction_file_paths, classification_file_paths = get_result_ex4cds_files(RESULT_PATH_ex4cds)
            dropdown_list.append(dcc.Dropdown(options=[{'label':d, 'value':d} for d in prediction_file_paths.keys()], id='selected-id', multi=False))
    return prediction_file_paths, classification_file_paths, dropdown_list

@callback([Output('tsv-or-line-sentences-memory', 'data'), 
           Output('cardio-section-checklist', 'options'),
           Output('cardio-section-checklist', 'value'),
            Output('prediction-memory', 'data'), 
           Output('classification-memory', 'data')], 
          [Input('select-domain', 'value'),
           Input('selected-id', 'value'), 
           Input('prediction-files-memory', 'data'), 
           Input('classification-files-memory', 'data')])
def return_cardio_tsv_sentences(domain, text_id, prediction_file_paths, classification_file_paths):
    section_options = []
    sentences_dict = {}
    prediction_dict = {}
    classification_dict = {}
    ...
    if domain == 'Kardiologie':
        if text_id and '_' not in text_id:
            ...
            section_options = make_section_options(domain, prediction_dict.keys())
           
            tsv_lines = get_tsv_lines(text_id)
            sentences_dict = retrieve_sentences(tsv_lines)
    else:
        if text_id and '_' in text_id: 
            prediction_dict, classification_dict, tokens = return_json_ex4cds_dict(text_id, RESULT_PATH_ex4cds, prediction_file_paths=prediction_file_paths, classification_file_paths=classification_file_paths)
            
            sec, current_file_id = get_ex4cds_text_id(classification_file_paths[text_id])
            word_spans, lines = get_ex4cds_text_lines(ex4cds_path=EX4CDS_PATH, section=sec, text_id=current_file_id)
            sentences_dict = {'word_spans' : word_spans, 'lines' : lines, 'tokens' : tokens}
    return sentences_dict, section_options, [], prediction_dict, classification_dict
        

@callback([Output('container-vis-tabs', 'children'),
           Output('container-vis-tabs', 'value'), 
           Output('section-tab-memory', 'data'),
           Output('spans-all-sentences-memory', 'data')],
        [Input('select-domain', 'value'),
         Input('selected-id', 'value'),
         Input('cardio-section-checklist', 'value'), 
         Input('vis-selected-types-memory', 'data'),
         Input('tsv-or-line-sentences-memory', 'data'),
         Input('prediction-memory', 'data'), 
         Input('classification-memory', 'data')], prevent_initial_call=True)
def make_section_tabs(domain, text_id, sections, selected_types,sentences_dict, prediction_current, classification_current):
        sec_sentences = {}
        section_tab = {}
        tabs = []
        labels = []
        dmc_tabs = []
        spans_all_sentences = []
        if domain == 'Kardiologie' and sections is not None:
            ...
            if sentences_dict and 'word_spans' in sentences_dict:
                spans_all_sentences = get_spans_all_sentences_ex4cds(**sentences_dict)
                spans_all_sentences = get_sentence_classification(classification_current, spans_all_sentences, selected_types, threshold='0.5')
                labels.append(dmc.Tab(text_id, value='tab-{}'.format(0)))
                lines_str_html_ths = ''
                for t, sent_spans in spans_all_sentences.items():
                ...
                one_tab = make_one_section_ner_tab('tab-{}'.format(0), lines_str_html_ths)
                tabs.append(one_tab)
                section_tab['tab-{}'.format(0)] = text_id
                
        dmc_tabs = [dmc.TabsList(labels)] + tabs
        return dmc_tabs, 'tab-0', section_tab, spans_all_sentences
        

@callback(Output('select-types-explain', 'children'), [Input('select-domain', 'value'),
                                                       Input('prediction-memory', 'data'),
                                                       Input('container-vis-tabs', 'value'),
                                                       Input('section-tab-memory', 'data')])
def make_explain_types_options(domain, prediction_dict,tab, sec_tab_memory):
    ...
    if  sec_tab_memory is not None and tab in sec_tab_memory:
        ...
        return html.Div([
                      dcc.Dropdown(id='types-options')
                      ])

@callback([Output('container-exp', 'children')],
                [Input('select-domain', 'value'),
                Input('prediction-memory', 'data'), 
                 Input('tsv-or-line-sentences-memory', 'data'),
                 Input('spans-all-sentences-memory', 'data'),
                 Input('types-options', 'value'), 
                 Input('container-vis-tabs', 'value'),
                 Input('section-tab-memory', 'data')],
                 prevent_initial_call=True)
def make_sentence_explanation(domain, scores_dict, tsv_sentences, spans_all_sentences, sem_types, tab, sec_tab_memory):
        ...
        if domain == 'Kardiologie' : 
            ...
            if spans_all_sentences is not None and sem_types is not None:
                word_scores = get_sentence_scores_by_type_ex4cds(scores_dict, spans_all_sentences, sem_types)
